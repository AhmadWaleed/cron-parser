...
func isValidRange(min, max, step uint, b bounds) error {
	var err error
	if min < b.min {
		err = fmt.Errorf("beginning of range (%d) below minimum (%d)", min, b.min)
	}
	if max > b.max {
		err = fmt.Errorf("end of range (%d) above maximum (%d)", max, b.max)
	}
	if min > max {
		err = fmt.Errorf("beginning of range (%d) beyond end of range (%d)", min, max)
	}
	if step == 0 {
		err = fmt.Errorf("step of range should be a positive number")
	}
	return err
}
